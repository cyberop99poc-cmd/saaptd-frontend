<template>
  <main class="p-8 space-y-8">
    ...
    <section class="space-y-6">
      <div class="bg-white rounded-2xl shadow-lg border border-violet-100/50 p-8">
        <h2
          class="text-2xl font-bold bg-gradient-to-r from-violet-600 to-purple-700 bg-clip-text text-transparent mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <input v-model="auditorName" type="text" placeholder="Masukkan nama juruaudit"
              class="w-full px-4 py-3 border border-violet-200/50 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-transparent outline-none transition-all text-sm bg-white/50 backdrop-blur">
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Tarikh Audit</label>
            <input v-model="auditDate" type="date"
              class="w-full px-4 py-3 border border-violet-200/50 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-transparent outline-none transition-all text-sm mono bg-white/50 backdrop-blur">
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">ID Audit</label>
            <input type="text" :value="auditId" readonly
              class="w-full px-4 py-3 border border-violet-200/50 rounded-xl bg-violet-50/50 text-gray-600 text-sm mono">
          </div>
        </div>
      </div>

      <!-- Audit Table -->
      <div class="bg-white rounded-2xl shadow-lg border border-violet-100/50 overflow-hidden">
        <div class="bg-gradient-to-r from-violet-600 to-purple-700 px-8 py-6 flex items-center justify-between">
          <div class="flex-1">
            <div class="flex items-center gap-3">
              <h2 class="text-xl font-bold text-white">🔍 Pengurusan Pendidikan - Senarai Semak Audit</h2>
            ...
            <p class="text-violet-100 text-sm mt-2">Tandakan setiap item berdasarkan pemeriksaan anda</p>
          </div>
          <div class="flex items-center gap-3">
            <div class="text-lg text-violet-100 font-semibold">Jumlah Markah</div>
            <span
        ...
        <div class="overflow-x-auto">
          <table class="w-full border-collapse text-sm">
            <thead>
              <tr class="bg-violet-50 border-b-2 border-violet-200">
              ...
              <tr class="bg-white border-b border-violet-100">
                <th colspan="3"></th>
            ...
            <tbody>
              <template v-for="item in flattenedItems" :key="`${item.key}`">
                <tr v-if="!item.isHeader && !item.isSubHeader"
                  :class="['border-b border-violet-50 hover:bg-violet-50/30 transition-colors']">
                  ...
                  <td v-for="score in [0, 1, 2, 3, 4, 5]" :key="score" class="px-2 py-3 text-center">
                    <input v-if="item.markahPenuh" type="radio" :name="`score-${item.key}`" :value="score"
                      ...
                      class="w-4 h-4 cursor-pointer text-violet-600 focus:ring-violet-500">
                  </td>
                ...
                <tr v-else-if="item.isSubHeader" :class="['bg-gray-50 border-b border-violet-50']">
                  <td class="px-4 py-3 font-bold">{{ item.bil }}</td>
                  <td class="px-4 py-3 text-gray-700 font-bold">{{ item.perkara }}</td>
                  <td colspan="8"></td>
                </tr>
                <tr v-else :class="['bg-gray-100 border-b-2 border-violet-200']">
        ...
        <div
          class="px-8 py-4 bg-gradient-to-r from-violet-50 to-purple-50 border-t border-violet-200/50 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="text-sm text-gray-700">
              Selesai: <span class="font-semibold text-violet-600">{{ completedCount }}</span> / <span>{{ totalItems
              }}</span>
              item
            </div>
            <div class="w-48 h-2.5 bg-gray-200 rounded-full overflow-hidden">
              <div class="h-full bg-gradient-to-r from-violet-600 to-purple-700 transition-all duration-300"
                :style="{ width: completionPercentage + '%' }"></div>
            </div>
          </div>
          <button @click="submitAudit()" :disabled="completedCount === 0"
            class="px-6 py-3 bg-gradient-to-r from-violet-600 to-purple-700 text-white font-semibold rounded-xl hover:shadow-lg transition-all shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
    ...
    <div v-if="showRujukanModal" @click.self="showRujukanModal = false"
      ...
      <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="p-8 overflow-y-auto flex-1">
          <div class="space-y-4">
            <div
              class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-xl p-6 border-l-4 border-blue-500 shadow-sm hover:shadow-md transition-shadow">
              <div class="flex items-start gap-4">
                <div class="bg-blue-500 text-white rounded-lg p-2 flex-shrink-0">
                  <span class="font-bold text-lg">A</span>
                </div>
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">PMAT 22/2002(A) – Pindaan Pertama Kepada PMAT 22/2002
                    Berkaitan
                    Dasar Pendidikan Angkatan Tentera.</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">PMAT 22/2002 – Dasar Pendidikan Angkatan Tentera.</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">PAAT 1/2003 – Pembelajaran Bahasa Inggeris Dalam ATM.</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">Arahan Pentadbiran Pendidikan, 2008.</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">BPA-DIDIK (NAZ).100-5/2/5-A (5) bertarikh 6 Mac 18 – Pindaan
                    Borang Pemeriksaan Pakar Pendidikan.</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">MATM/J1PENDIDIKAN/6377/1 Jil 6 (38) bertarikh 20 Okt 04 –
                    Pindaan
                    Peraturan Tabung Pinjaman Akademik dan Pengajian Tinggi Angkatan Tentera (TAPAT) dan Skim Pinjaman
                    Akademik Anak Tentera (SPAAT).</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">MATM/J1PENDIDIKAN/6377/1 Jil 6 (19) bertarikh 20 Apr 04 –
                    Peraturan Skim Tabung Pinjaman Akademik dan Pengajian Tinggi Angkatan Tentera (TAPAT).</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">BPA-DIDIK (PMET).500-13/7/6(1) bertarikh 11 Feb 20 – Arahan
                    Perlaksanaan Kursus Pendidikan Moral dan Etika Ketenteraan (PMET) ATM Mulai Tahun 2020.</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">BPA-DIDIK (PAM).100-3/3/4-2 (01) bertarikh 30 Jan 20 – Garis
                    Panduan Perlaksanaan Kelas Bimbingan Untuk Anak-Anak Anggota Tentera.</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  <p class="text-gray-800 leading-relaxed">MK TD/G1/1117/(PEG) bertarikh 16 Jul 99 - Penguasaan Bahasa
                    Inggeris.</p>
                </div>
              </div>
            </div>
...
const auditId = computed(() => `AUD-PDK-${Date.now()}`)
const currentPhase = ref<'self' | 'commander' | 'aptd'>('self')
const auditScores = ref<AuditScore>({})
const toastVisible = ref(false)
const toastMessage = ref('')
const toastType = ref<'success' | 'error'>('success')
const formType = 'Pengurusan Pendidikan'
const penilaianTypeId = 10
...
const draftKeyForPhase = (phaseSlug: string) => `pendidikan_auditScoresDraft_${phaseSlug}`
...
const auditItems: AuditItem[] = [
    ...
    subItems: [
      {
        ...
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'PMAT 22/2002 dan 22/2002(A).' },
          { bil: '(2)', perkara: 'PAAT 1/2003.' },
          { bil: '(3)', perkara: 'Arahan Pentadbiran Pendidikan, 2008.' },
          { bil: '(4)', perkara: 'Direktif Panglima Formasi.' }
        ]
      }
    ]
  },
  {
    bil: '2',
    perkara: 'Pengurusan Kegiatan Pendidikan:',
    isHeader: true,
    subItems: [
      {
        bil: 'a',
        ...
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Carta Organisasi Pejabat Pendidikan dipamer dan kemas kini.' },
          { bil: '(2)', perkara: 'Pasukan mempunyai perjawatan dan pegangan anggota KPA (Pendidikan).' },
          { bil: '(3)', perkara: 'Penyata keanggotaan KPA (Pendidikan) pasukan adalah kemas kini.' },
          { bil: '(4)', perkara: 'Perintah Tetap anggota KPA (Pendidikan) dipamer dan kemas kini.' },
          { bil: '(5)', perkara: 'Pengurusan sistem pemfailan bagi pendidikan adalah teratur.' },
          { bil: '(6)', perkara: 'Sasaran Kerja Tahunan (SKT) bagi anggota KPA (Pendidikan) disediakan.' },
          { bil: '(7)', perkara: 'Rancangan aktiviti pendidikan pasukan disediakan.' },
          { bil: '(8)', perkara: 'Kebersihan, pengudaraan dan keceriaan ruang pejabat adalah memuaskan.' }
        ]
      },
      {
        bil: 'b',
        perkara: 'Rekod dan data:',
        markahPenuh: 0.50,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Rekod/data peribadi pendidikan bagi anggota dan keluarga diwujudkan.' },
          { bil: '(2)', perkara: 'Buku kehadiran aktiviti pendidikan adalah kemas kini.' },
          { bil: '(3)', perkara: 'Buku rekod sijil penyelesaian disediakan.' },
          { bil: '(4)', perkara: 'Statistik kelulusan akademik anggota disediakan.' },
          { bil: '(5)', perkara: 'Statistik anak-anak anggota di TADIKA/Pra Sekolah disediakan.' },
          { bil: '(6)', perkara: 'Statistik anak-anak anggota di sekolah rendah disediakan.' },
          { bil: '(7)', perkara: 'Statistik anak-anak anggota di sekolah menengah disediakan.' },
          { bil: '(8)', perkara: 'Statistik anak-anak anggota di IPTA/IPTS disediakan.' },
          { bil: '(9)', perkara: 'Rekod anak-anak anggota menduduki peperiksaan SPM diwujudkan.' },
          { bil: '(10)', perkara: 'Rekod keputusan peperiksaan SPM anak-anak anggota diwujudkan.' },
          { bil: '(11)', perkara: 'Statistik menduduki peperiksaan SPM diwujudkan.' },
          { bil: '(12)', perkara: 'Statistik keputusan peperiksaan SPM diwujudkan.' }
        ]
      },
      {
        bil: 'c',
        perkara: 'Pemeriksaan Pakar Pendidikan:',
        markahPenuh: 0.20,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Jadual Pemeriksaan Pakar Pendidikan diterima daripada formasi atasan.' },
          { bil: '(2)', perkara: 'Pemeriksaan Pakar Pendidikan dilakukan sekali dalam jangka masa 18 bulan.' },
          { bil: '(3)', perkara: 'Laporan Pemeriksaan Pakar Pendidikan yang lepas telah diterima.' },
          { bil: '(4)', perkara: 'Laporan Pemeriksaan Pakar Pendidikan telah diambil tindakan.' },
          { bil: '(5)', perkara: 'Penyata/statistik pencapaian Pemeriksaan Pakar Pendidikan direkodkan.' }
        ]
      }
    ]
  },
  {
    bil: '3',
    perkara: 'Pentadbiran Hal Ehwal Pendidikan:',
    isHeader: true,
    subItems: [
      {
        bil: 'a',
        perkara: 'Anugerah Kecemerlangan Pendidikan:',
        markahPenuh: 0.20,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Hebahan dalaman berkaitan hal ehwal pendidikan dibuat.' },
          { bil: '(2)', perkara: 'Borang dan peraturan permohonan disediakan.' },
          { bil: '(3)', perkara: 'Rekod keputusan peperiksaan UPSR/PT3/SPM pemohon.' },
          { bil: '(4)', perkara: 'Rekod senarai penerima/pemohon.' },
          { bil: '(5)', perkara: 'Statistik penerima.' }
        ]
      },
      {
        bil: 'b',
        perkara: 'Dermasiswa Tabung Pendidikan Kanak-Kanak Angkatan Tentera (TPKKAT):',
        markahPenuh: 0.20,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Hebahan dalaman dibuat.' },
          { bil: '(2)', perkara: 'Borang dan peraturan permohonan disediakan.' },
          { bil: '(3)', perkara: 'Rekod Nombor Keahlian.' },
          { bil: '(4)', perkara: 'Rekod permohonan ahli baru.' },
          { bil: '(5)', perkara: 'Statistik keahlian tabung.' }
        ]
      },
      {
        bil: 'c',
        perkara: 'Kelas Bimbingan:',
        markahPenuh: 0.20,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Arahan pentadbiran dikeluarkan.' },
          { bil: '(2)', perkara: 'Hebahan dalaman dibuat.' },
          { bil: '(3)', perkara: 'Rekod tuntutan/pembayaran subsidi disediakan.' },
          { bil: '(4)', perkara: 'Senarai peserta direkodkan.' },
          { bil: '(5)', perkara: 'Buku kehadiran kelas disediakan.' },
          { bil: '(6)', perkara: 'Laporan kelas bimbingan dibuat.' }
        ]
      },
      {
        bil: 'd',
        perkara: 'Tabung Pinjaman Akademik dan Pengajian Tinggi (TAPAT):',
        markahPenuh: 0.20,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Hebahan dalaman dibuat.' },
          { bil: '(2)', perkara: 'Borang dan peraturan permohonan disediakan.' },
          { bil: '(3)', perkara: 'Rekod/Senarai penerima/pemohon disediakan.' }
        ]
      },
      {
        bil: 'e',
        perkara: 'Skim Pinjaman Akademik Anak-Anak Tentera (SPAAT):',
        markahPenuh: 0.20,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Hebahan dalaman dibuat.' },
          { bil: '(2)', perkara: 'Borang dan peraturan permohonan disediakan.' },
          { bil: '(3)', perkara: 'Rekod/senarai penerima/pemohon.' }
        ]
      }
    ]
  },
  {
    bil: '4',
    perkara: 'Skim Pengajian Tinggi ATM:',
    isHeader: true,
    subItems: [
      {
        bil: 'a',
        perkara: 'Hebahan dalaman dibuat.',
        markahPenuh: 0.10,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Hebahan dalaman dibuat.' },
  ...
  {
    bil: '5',
    perkara: 'Pengurusan Perpustakaan:',
    isHeader: true,
    subItems: [
      {
        bil: 'a',
        ...
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Perintah Tetap Perpustakaan dipamerkan dan kemas kini.' },
          { bil: '(2)', perkara: 'Semua buku direkodkan dalam lejar.' },
          { bil: '(3)', perkara: 'Pemeriksaan Stok perpustakaan dibuat.' },
          { bil: '(4)', perkara: 'Perolehan buku (IV, lejar dan katalog).' },
          { bil: '(5)', perkara: 'Keselamatan buku (tiada kehilangan).' },
          { bil: '(6)', perkara: 'Kemudahan Perpustakaan (sesuai atau tidak).' },
          { bil: '(7)', perkara: 'Pelupusan.' },
          { bil: '(8)', perkara: 'Keselesaan (bersih, ruang, pengudaraan dan keceriaan).' }
        ]
      },
      {
        bil: 'b',
        ...
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Sistem pengkatalogan dan pengkelasan ke atas buku-buku dibuat.' },
          { bil: '(2)', perkara: 'Rekod pinjaman dan pemulangan (rekod daftar) diwujudkan.' },
          { bil: '(3)', perkara: 'Peraturan perpustakaan disedia dan dipamerkan.' },
          { bil: '(4)', perkara: 'Sistem maklumat buku dibuat (manual/berkomputer).' },
          { bil: '(5)', perkara: 'Kemudahan internet (jika berkaitan)' },
          { bil: '(6)', perkara: 'Pinjaman antara perpustakaan (jika perlu).' },
          { bil: '(7)', perkara: 'Hebahan buku (promosi) dibuat.' },
          { bil: '(8)', perkara: 'Akhbar dan majalah disediakan.' },
          { bil: '(9)', perkara: 'Buku kehadiran diwujudkan.' },
          { bil: '(10)', perkara: 'Buku cadangan pengguna disediakan.' }
        ]
      },
      {
        bil: 'c',
        perkara: 'Perkakasan dan perisian SPD ATM (MK Formasi/ Pemerintahan sahaja):',
        markahPenuh: 0.10,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'BAT L 5/Lejar/KEW. PA.' },
          { bil: '(2)', perkara: 'Rekod senggaraan dibuat.' },
          { bil: '(3)', perkara: 'Pemeriksaan stok dilaksanakan.' },
          { bil: '(4)', perkara: 'Buku log penggunaan terminal internet.' },
          { bil: '(5)', perkara: 'Perintah Tetap penggunaan terminal internet.' },
          { bil: '(6)', perkara: 'Anggota mempunyai kursus SPD ATM.' },
          { bil: '(7)', perkara: 'Laporan status SPD ATM.' },
          { bil: '(8)', perkara: 'Rekod pembayaran bil internet.' },
          { bil: '(9)', perkara: 'Buku cadangan pengguna.' }
        ]
      }
    ]
  },
  {
    bil: '6',
    perkara: 'Pengurusan Sekolah dan TABIKA Dalam Kem ATM (jika berkaitan):',
    isHeader: true,
    subItems: [
      {
        bil: 'a',
        ...
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Buku rekod lawatan ke sekolah disediakan.' },
          { bil: '(2)', perkara: 'Statistik pelajar dan guru disediakan.' },
          { bil: '(3)', perkara: 'Skop senggaraan yang dimajukan.' },
          { bil: '(4)', perkara: 'Skop senggaraan yang diluluskan (tahun semasa).' },
          { bil: '(5)', perkara: 'Rekod Harta Modal.' },
          { bil: '(6)', perkara: 'Seliaan Harta Modal.' },
          { bil: '(7)', perkara: 'Rekod penambahbaikan/pelupusan Harta Modal.' },
          { bil: '(8)', perkara: 'Mesyuarat PIBG (minit/laporan).' },
          { bil: '(9)', perkara: 'Laporan senggaraan tahunan.' }
        ]
      },
      {
        bil: 'b',
        ...
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Hebahan pendaftaran TABIKA dalam/luar pasukan.' },
          { bil: '(2)', perkara: 'Rekod pelajar dan guru TABIKA.' },
          { bil: '(3)', perkara: 'Statistik pelajar dan guru TABIKA.' },
          { bil: '(4)', perkara: 'Mesyuarat TABIKA (minit/laporan).' },
          { bil: '(5)', perkara: 'Akaun TABIKA.' },
          { bil: '(6)', perkara: 'Peruntukan Kerajaan Pusat/Negeri dan Lain-Lain.' },
          { bil: '(7)', perkara: 'Pembelian dari peruntukan ATM.' },
          { bil: '(8)', perkara: 'Rekod perbelanjaan TABIKA.' },
          { bil: '(9)', perkara: 'Rekod Harta Modal.' },
          { bil: '(10)', perkara: 'Penyeliaan Harta Modal.' },
          { bil: '(11)', perkara: 'Akaun perbelanjaan harian.' },
          { bil: '(12)', perkara: 'Buku Rekod Lawatan TABIKA.' }
        ]
      }
    ]
  },
  {
    bil: '7',
    perkara: 'Penguasaan Bahasa Inggeris:',
    isHeader: true,
    subItems: [
      {
        bil: 'a',
        perkara: 'Aktiviti bahasa Inggeris yang dilaksanakan.',
        markahPenuh: 0.20,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Aktiviti bahasa Inggeris yang dilaksanakan.' },
          { bil: '(2)', perkara: 'Kelas/kursus bahasa Inggeris.' },
          { bil: '(3)', perkara: 'Rekod anggota menghadiri kursus bahasa Inggeris anjuran ATM/agensi luar/persendirian.' },
          { bil: '(4)', perkara: 'Statistik anggota menghadiri kursus bahasa Inggeris anjuran ATM/agensi luar/persendirian.' }
        ]
      }
    ]
  },
  {
    bil: '8',
    perkara: 'Penguasaan Moral dan Etika Tentera (PMET):',
    isHeader: true,
    subItems: [
      {
        bil: 'a',
        perkara: 'Senarai nama anggota.',
        markahPenuh: 0.30,
        isSubHeader: true,
        subItems: [
          { bil: '(1)', perkara: 'Senarai nama anggota.' },
          { bil: '(2)', perkara: 'Jadual kelas PMET.' },
          { bil: '(3)', perkara: 'Buku kehadiran kelas PMET.' },
          { bil: '(4)', perkara: 'Rekod keputusan ujian PMET.' },
          { bil: '(5)', perkara: 'Statistik kehadiran ujian PMET.' },
          { bil: '(6)', perkara: 'Statistik keputusan ujian PMET.' }
        ]
      }
    ]
  }
...
watch(() => currentPhase.value, (newPhase: string) => {
  loadScoresDraft(newPhase as any)
})
